  ...
  const fetchTripByVehicle = useCallback(async (requestedVehicle) => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        console.error('Vehicle tracking authorization failed:', error);
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Monitor active vehicle routes and alerts.</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        {role === 'OWNER' && <form onSubmit={handleTrackVehicle} className="flex flex-col gap-2 sm:flex-row sm:items-center">
